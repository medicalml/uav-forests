...
class RGB_NDVI_RetinaNet(RetinaNet):
    def __init__(self, cfg):
        ...
        pixel_mean = torch.Tensor(cfg.MODEL.PIXEL_MEAN).to(self.device).view(image_channel_nr, 1, 1)
        pixel_std = torch.Tensor(cfg.MODEL.PIXEL_STD).to(self.device).view(image_channel_nr, 1, 1)
